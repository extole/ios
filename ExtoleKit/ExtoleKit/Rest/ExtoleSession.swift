...
@objc public final class ExtoleSession: NSObject{
    ...
    let token: ExtoleAPI.Authorization.ConsumerToken
    var baseUrl: URL {
        get {
   ...
   func deleteRequest(url: URL) -> URLRequest {
    ...
    init(program: ExtoleAPI, token: ExtoleAPI.Authorization.ConsumerToken) {
    ...
    public func verify(success : @escaping (_: ExtoleAPI.Authorization.ConsumerToken) -> Void,
                         error: @escaping (_: ExtoleError) -> Void) {
        let url = URL.init(string: token.access_token, relativeTo: ExtoleSession.v4TokenUrl(baseUrl: baseUrl))!
